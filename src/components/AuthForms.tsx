...
export function AuthForm({ mode }: { mode: Mode }) {
  ...
  return (
    <form onSubmit={submit} className="glass mx-auto w-full max-w-lg space-y-4 rounded-2xl p-6 shadow-premium">
      ...
      <button disabled={loading} className="w-full rounded-lg bg-night px-5 py-3 font-bold text-white transition hover:scale-[1.01] disabled:opacity-60">
        {loading ? "Traitement..." : mode === "register" ? "Créer mon compte démo" : "Se connecter"}
      </button>
    </form>
  );
}
