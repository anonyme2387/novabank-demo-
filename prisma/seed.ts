...
async function main() {
  ...
  for (const account of accounts) {
    await prisma.transaction.createMany({
      data: [
        { accountId: account.id, type: TransactionType.DEPOSIT, amount: "1000.00", label: "Dépôt initial" },
        { accountId: account.id, type: TransactionType.WITHDRAWAL, amount: "45.50", label: "Retrait carte" },
        { accountId: account.id, type: TransactionType.DEPOSIT, amount: "220.00", label: "Prime NovaBank" }
      ]
    });
  }
